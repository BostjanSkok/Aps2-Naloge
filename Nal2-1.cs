...
void Main()
{
	...
	var root = BuildTree<int>(preOrderC,inOrderC);
	Draw<int>(root,@"C:\Users\BostjanSkok\Documents\New folder (4)\BinaryTreeNode.dgml");
}
private BinaryTreeNode<T> BuildTree<T>(T[] preOrd, T[] inOrd)
{
  return BuildTreeR(inOrd, 0, inOrd.Length - 1, preOrd, 0);
}

private BinaryTreeNode<T> BuildTreeR<T>(T[] inOrd, int inStart, int inEnd, T[] preOrd, int preStart)
{
    //found leaf return node without children
    if (inStart == inEnd) return new BinaryTreeNode<T>(inOrd[inStart]);
    // handled all nodes in subtree
    if (inStart > inEnd) return null;

    int indexOfRoot;
    T root = preOrd[preStart];
    //search for root index in inOrder table
    for (indexOfRoot = inStart; indexOfRoot < inEnd; indexOfRoot++) 
        if (root.Equals(inOrd[indexOfRoot])) break;
    ...
    return new BinaryTreeNode<T>
    {
        ...
        Left = BuildTreeR<T>(inOrd, inStart, indexOfRoot - 1, preOrd, preStart + 1),
        //Build Right subtree
        ...
        Right = BuildTreeR<T>(inOrd, indexOfRoot + 1, inEnd, preOrd,
